...
krokiet_info_message =
    O Krokiet (Croquete) é a nova versão aprimorada e mais rápida, que possui menos problemas ou falhas do que o Czkawka (Soluço), que possui a sua interface gráfica desenvolvida com o conjunto de ferramentas do GTK.
    
	Por ser independente, é muito mais fácil de executar e mais resistente a alterações do sistema e a atualizações da versão dos pacotes.
    
	
    O Krokiet inclui muitas funcionalidades que não estão disponíveis no Czkawka, como a exibição das miniaturas no modo de comparação dos vídeos, exibe o progresso das ações de mover/copiar/excluir os arquivos e as opções de organização (ordenação ou classificação) da exibição dos arquivos.
    
    Experimente o Krokiet e veja as diferença em relação ao Czkawka.
    
    O Czkawka continuará recebendo novas correções e melhorias de falhas ou problemas, mas todas as novas funcionalidades serão implementadas apenas no Krokiet.
    
    Observação: Esta mensagem deve aparecer apenas uma vez, mas se aparecer novamente, defina a variável de ambiente CZKAWKA_DONT_ANNOY_ME com qualquer valor que não seja vazio ou em branco.
# Main window
...
same_music_tooltip =
    A pesquisa de arquivos de música equivalentes por seu conteúdo pode ser defininda por meio das configurações:
...
image_resize_filter_tooltip =
    ...
    O algoritmo mais rápido é o que produz os piores resultados e está ativado por padrão, porque o ‘hash’ com um tamanho de 16x16 a sua qualidade não é realmente perceptível.
...
upper_excluded_items_tooltip =
    Os itens que serão ignorados tem que conter o asterisco ‘ * ’, que corresponde a qualquer cadeia de caracteres e tem que ser separados por vírgulas.
...
bottom_select_button_tooltip = Selecionar os registros. Apenas os arquivos e as pastas que foram selecionadas poderão ser processadas posteriormente.
...
# About Window
about_repository_button_tooltip = Endereço da página eletrônica do repositório com o código-fonte do programa Czkawka/Krokiet.
about_donation_button_tooltip = Endereço da página eletrônica para fazer uma doação ao programador do Czkawka/Krokiet.
about_instruction_button_tooltip = Endereço da página eletrônica para obter ajuda.
about_translation_button_tooltip = Endereço da página eletrônica da plataforma de tradução ‘Crowdin’ com as traduções do programa Czkawka/Krokiet. Os idiomas polonês e inglês são fornecidos oficialmente pelo Rafał Mikrut, que também é conhecido por ‘qarmin’ (https://github.com/qarmin) e o idioma português do Brasil foi gentilmente traduzido por marcelocripe (https://github.com/marcelocripe e https://gitlab.com/marcelocripe) em 2024, 2025 e 2026.
...
settings_folder_settings_open_tooltip =
    ...
    Aviso. A modificação manual das configurações pode interromper o seu fluxo de trabalho.
...
compute_found_duplicates_hash_size = Foram encontrados { $number_files } arquivos duplicados nos { $number_groups } grupos e ocupou o tamanho { $size }. A verificação durou { $time }
...
compute_found_images = Foram encontradas { $number_files } arquivos de imagens equivalentes nos { $number_groups } grupos. A verificação durou { $time }
...
progress_scanning_broken_files = Pesquisando { $file_checked } de { $all_files } no arquivo ({ $data_checked } de { $all_data })
...
progress_scanning_image = O ‘hash’ de { $file_checked } de { $all_files } da imagem ({ $data_checked } de { $all_data })
...
progress_scanning_music_content = Foi calculado a impressão digital do arquivo de música { $file_checked } de { $all_files } e foi verificado ({ $data_checked }/{ $all_data }) 
...
krokiet_new_app = O Czkawka está em modo de manutenção, o que significa que apenas os problemas críticos serão corrigidos e nenhuma nova funcionalidade será adicionada ao programa. Para obter as novas funcionalidades, por favor, confira o novo programa chamado de Krokiet, que é mais estável, mais eficiente e ainda está em desenvolvimento ativo.
# Various dialog
dialogs_ask_next_time = Perguntar novamente na próxima vez que for exibida a janela
...
hard_sym_invalid_selection_label_2 = Para criar uma ligação simbólica rígida dos arquivos, pelo menos dois registros de um grupo tem que ser selecionados.
...
search_not_choosing_any_music = Ocorreu um erro. Você tem que selecionar pelo menos uma caixa de seleção com o tipo dos arquivos de música que serão pesquisados.
search_not_choosing_any_broken_files = Ocorreu um erro. Você tem que selecionar pelo menos uma caixa de seleção com o tipo dos arquivos corrompidos que serão pesquisados.
