...
krokiet_info_message =
    O Krokiet (Croquete) é a nova versão aprimorada, mais rápida e confiável, que possui menos problemas ou falhas do que o Czkawka (Soluço), que possui a sua interface gráfica desenvolvida com o conjunto de ferramentas do GTK.

	Por ser independente, é muito mais fácil de executar e mais resistente a alterações do sistema, pois depende apenas das bibliotecas essenciais que estão disponíveis na maioria dos sistemas.

	O Krokiet inclui muitas funcionalidades que não estão disponíveis no Czkawka, como a exibição das miniaturas no modo de comparação dos vídeos, um removedor de ‘EXIF’ dos arquivos, exibe o progresso das ações de mover/copiar/excluir os arquivos e as opções de organização (ordenação ou classificação) da exibição dos arquivos.
	
	Experimente o Krokiet e veja as diferença em relação ao Czkawka.

	O Czkawka continuará recebendo novas correções e melhorias de falhas ou problemas, mas todas as novas funcionalidades serão implementadas apenas no Krokiet e qualquer pessoa pode contribuir com novas funcionalidades, adicionar os modos que estão ausentes ou ampliar ainda mais as funcionalidades do Czkawka.

	Observação: Esta mensagem deve ser exibida apenas uma vez, mas se for exibida novamente, defina a variável de ambiente CZKAWKA_DONT_ANNOY_ME com qualquer valor que não seja vazio ou em branco.
...
upper_excluded_items_tooltip = 
    ...
    Esta opção é mais lenta do que a opção do ‘Caminho dos diretórios não incluídos’, portanto, deve ser utilizada com muito cuidado.
...
bottom_sort_button_tooltip =
        Ordenar os arquivos ou os diretórios de acordo com o método que foi selecionado.
bottom_compare_button_tooltip =
        Comparar os arquivos e os diretórios nos grupos.
...
progress_scanning_empty_folders = {$folder_number -> 
        [one] Foi verificado ‘{$file_number}’ pasta
       *[other] Foram verificados ‘{$file_number}’ pastas
}
